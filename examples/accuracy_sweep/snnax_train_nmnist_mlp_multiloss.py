...
def get_heaviside_with_super_spike_surrogate(beta=10., first_minus_second_threshold=0.05):
    @jax.custom_jvp
    ...
    def f_jvp(primals, tangents):
        ...
        tangent_out = 1.0 / (beta*jnp.abs(x) + 1.0) * x_dot * jnp.heaviside(x+first_minus_second_threshold, 0) # TODO check if this is correct, square and beta missing ?
...
class SNN(eqx.Module):
    ...
    def __init__(self, 
        ...
        surr = get_heaviside_with_super_spike_surrogate(beta=10., first_minus_second_threshold=1.0-kwargs['second_threshold'])
