...
def main():
    ...
    for element in selected_runs:
        print(element["use_sparse"].iloc[0], len(element))

    # only use runs with same hyperparameters
    skwargs_runs_df = pd.concat(selected_runs)
    # skwargs_runs_df = runs_df

    print(skwargs_runs_df)
    print(len(skwargs_runs_df))


    dense_df = subselect(skwargs_runs_df, use_sparse=False)
    sparse_df = subselect(skwargs_runs_df, use_sparse=True)
    if 3*len(dense_df) != len(sparse_df):
        print("len(dense_df)", len(dense_df))
        print("len(sparse_df)", len(sparse_df))
        raise RuntimeError("number of dense runs must be 1/3 of sparse runs")
    ...
    for USE_AUG in [True, False, None]:
        if USE_AUG is None:
